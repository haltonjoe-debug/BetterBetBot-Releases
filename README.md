...
Combine multiple bets with `+` for multiplied odds:

```
!bet 100 acc:p1+under4x
!bet 50 acc:podium+finish+under2x
```
...
BetterBetBot can post race results, betting odds, and live notifications to your Discord server — complete with generated graphics and interactive betting buttons.

### What You Get

- **Race Result Cards** — After each race, a generated image with track, position, bet summary, and winner leaderboard is posted to your results channel
- **Live Betting** — When betting opens, an interactive message with buttons appears so Discord users can place bets directly
- **Slash Commands** — `/leaderboard`, `/balance`, `/odds`, `/stats`, `/recent` available in your server
- **Green Flag / Betting Closed** notifications in real time
- **Log Upload** — One-click button in the GUI to send your log files to a private Discord channel for support

### Setup (5 minutes)

#### Step 1: Enable Developer Mode in Discord

1. Open **Discord** → click the ⚙️ gear icon (User Settings)
2. Go to **Advanced** (in the left sidebar, near the bottom under "App Settings")
3. Toggle **Developer Mode** to **ON**

> This lets you right-click things in Discord to copy their IDs.

#### Step 2: Invite the Bot to Your Server

1. Open BetterBetBot → go to the **Settings** tab
2. Scroll down to **Discord Integration**
3. Click **"Invite Bot to Discord Server"**
4. In the browser, select your Discord server from the dropdown
5. Click **Authorize**
6. The bot should now appear in your server's member list

#### Step 3: Create Your Channels

Create these channels in your Discord server (or use existing ones):

| Channel | Purpose | Suggested Name |
|---|---|---|
| **Results Channel** | Race result cards + settlement summaries | `#race-results` |
| **Betting Channel** | Betting-open messages with buttons, green flag alerts | `#betting` |
| **Logs Channel** *(optional)* | Private channel for log file uploads | `#support-logs` |

#### Step 4: Copy Your IDs

1. **Server (Guild) ID:** Right-click your server name (top of channel list) → **Copy Server ID**
2. **Results Channel ID:** Right-click `#race-results` → **Copy Channel ID**
3. **Betting Channel ID:** Right-click `#betting` → **Copy Channel ID**
4. **Logs Channel ID** *(optional)*: Right-click `#support-logs` → **Copy Channel ID**

#### Step 5: Configure in BetBot

1. In BetterBetBot → **Settings** tab → **Discord Integration**
2. Paste each ID into the matching field:
   - **Guild (Server) ID** → the server ID you copied
   - **Results Channel** → the results channel ID
   - **Betting Channel** → the betting channel ID
   - **Logs Channel** → the logs channel ID (optional)
3. Check the **"Enable Discord Bot"** checkbox
4. Click **"Save Discord Settings"**
5. **Restart BetterBetBot**

#### Step 6: Set Up Log Uploads (Optional)

If you want the **"Send Logs to Discord"** button to work:

1. In Discord, go to your `#support-logs` channel
2. Click the ⚙️ gear icon → **Integrations** → **Webhooks**
3. Click **"New Webhook"** → name it anything (e.g. "BetBot Logs")
4. Click **"Copy Webhook URL"**
5. In BetBot → **Settings** → paste the URL into **"Logs Webhook URL"**
6. Click **"Save Discord Settings"**

Now you can click **"Send Logs to Discord"** anytime to upload sanitized log files.

### Discord Slash Commands

Once the bot is running, these commands are available in your server:

| Command | Description |
|---|---|
| `/leaderboard` | Shows a generated image of the top 15 bettors by balance |
| `/balance username:name` | Look up a Twitch user's betting balance and record |
| `/odds` | Shows current betting odds as a generated image |
| `/stats` | Shows overall betting statistics (total races, bets, payouts) |
| `/recent count:10` | Shows the most recent settled bets |

### FAQ

**Q: Does the bot only show MY race bets?**
Yes. Each BetBot instance has its own database. The Discord bot only posts results from your races — never anyone else's.

**Q: Can my viewers bet through Discord?**
Yes! When betting opens, the bot posts a message with buttons for each bet type. Discord users click a button to place a bet. They get their own balance separate from Twitch users.

**Q: Do I need to create my own Discord bot?**
No. BetterBetBot uses a shared bot (same model as the Twitch bot). Just invite it and configure your channel IDs.
...
<p align="center">
  Built by <a href="https://www.twitch.tv/joehalton">Joe Halton</a> &middot; BetterBetBot v1.0.0
</p>
